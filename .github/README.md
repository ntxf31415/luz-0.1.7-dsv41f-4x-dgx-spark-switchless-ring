...
| # | 改动 | 类型 | **影响** | 理由 / 证据 |
|---|---|---|---|---|
| 1 | **摘除 `DSV41_MOE_B12X` 族**（`_B12X` / `_CAPS` / `_QUANT`） | **引擎行为** | **性能：更快 +17~25%**（decode 口径），且恢复输出可复现 | 实测两个量化档都更慢、且让贪心输出不可复现（见 §二） |
| 2 | `NCCL_DEBUG_FILE` → `/state/…` | 运维 | 无（但环自检依赖它） | 上游 `start.sh` 只在**开发模式**挂 `/nccl-debug`；生产模式日志无处落、环自检只能判失败 |
| 3 | `NCCL_IB_GID_INDEX_FORCE=-1` | 环境 | **连通性**：填错 = 栈起不来 | 本机 kernel-1031 GID 表重排，禁止自动探测（上游模板为 `3`） |
| 4 | served-name 双别名 | 兼容 | 无 | 下游门户在用旧名 `deepseek-v4-flash-vision-exp`，去掉即断链 |
| 5 | 四机本地权重 + `WORKER_ENGRAM_DIR` 显式指向 | 环境 | 仅**启动时间**（免重打 engram） | 站点四机全本地；复用现成 48 GB engram 分片，免重打 |
| 6 | `site/tools/` 基准工具集 | **新增代码** | 无 | 上游没有：`armsuite.sh` / `night-matrix.sh` / `pr-matrix.sh` / `ab-arm.sh` 等 |
| 7 | `site/results/` 原始产物 | **新增数据** | 无 | SD-1 DE 矩阵 4 类型 × 5 并发 × 3 波；PR-v3 24 格；b12x A/B；16K/64K 补测 |
| 8 | `site/config/.env.tp4.example` | 新增配置 | 无（脱敏模板） | 站点生产配置（已脱敏） |
| 9 | **`site/patches/`：autotune 选型钉住补丁** | **引擎行为** | **性能：消跨 boot 方差** —— 不加则同配置跨 boot 差 **1.5 倍** | 上游 SGLang 在 EP 分片下每 boot 删掉 autotune 缓存 ⇒ fused-MoE 选型重新抽签（见 §六） |
| 10 | `site/tools/perf-gate.sh` + 挂进 monitor | 运维 | 无（测量行为） | 每次新 boot 自动跑一次性能门禁（口径 D 8K/100K + PR-v3 131072×C1） |
| 11 | `site/tools/` 新增分析/修补脚本 | **新增代码** | 无 | `pr-v3-conical.sh` / `merge_pr_v3_batches.py` / `make_flashinfer_autotune_patch.py` / `make_entropy_variant.py` 等 |
| 12 | **`--prefill-decode-interval 8`**（PDI） | **引擎行为** | **性能：改延迟分配**（在途解码流 5% → 41%）；独跑冷加载与并发阶梯**无影响** | 冷长 prefill 期间在途解码流被冻到基线的 **5%**；加此旋钮回到 **41%**，而**独跑冷加载零成本**（见 §七） |
| 13 | `site/tools/` 再增：`pdi-storm-probe.py` / `fingerprint.py` / `prose_bench_sparkdash.py` | **新增代码** | 无 | 冻结比探针（口径 H）· 贪心指纹（改输出的改动一律先过它）· sparkDash 口径的散文尺 |
| 14 | `site/tools/patch_gsm8k_thinking.py` | **口径修正** | 无（防静默漂移） | GSM8K harness 显式钉死 `thinking=false`，消除对引擎默认值的隐式依赖（与上游 PR #1 同款修复） |
| 15 | `site/tools/patch_hc_combine_96.py` / `patch_start_sh_hc96.py` | **证据留档** | 无（**未挂载**） | 上游 `#40208`（融合 `hc_combine_norm` 扩到 9–96 行）的移植件与挂载器 —— 实测**改输出**（指纹 `13a18f7a…`→`ab56c6d6…`），按纪律**否决并撤回**，脚本保留可复核 |

**影响一览**：**只有 #1 / #9 / #12 是性能项** —— #1 让本站**更快**（+17~25% decode）、#9 让本站**更稳**（消掉 1.5 倍的跨 boot 摆动）、#12 改善**并发受害者的延迟**（吞吐不变）；**#3 是"能不能跑起来"**（连通性）；其余 11 项是环境 / 工具适配，**不改变性能**。

⚠️ #1 的 +17~25% 是**同一 boot 内**的 A/B 结果，可直接引用；**跨栈（本站 vs 上游）的绝对差别不能这么读** —— 上游引擎可能落在慢 boot（见 §六），且本站 PR-v3 长档领先已按纪律降级为线索。
