#!/bin/bash
# arm-chunk.sh <tag> -- fingerprint, mixed probe, PR-v3 24, DE 20, long-档 D (+mem sample), gate --full.
set -uo pipefail
cd "$HOME/luz017"
...
echo "--- [2] mixed-queue probe (share=0 baseline) ---"
python3 scripts/verify/mixed-queue-probe.py
...
echo "--- [5] long-档 D 563000 + memory sample (T2 evidence) ---"
